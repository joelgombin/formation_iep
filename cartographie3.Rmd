...
fonds <- read_osm(bb(departements, ext = 1.2), minNumTiles = 10)
...
tm_shape(departements) +
  tm_borders(col = "white") +
  tm_fill(col = c("Hollande2.exp", "Sarkozy2.exp"), alpha = 0.8, style = "quantile", n = 6, palette = "Reds", legend.format = list(text.separator = "à", digits = 1), title = c("Sarkozy 2e tour", "Hollande 2e tour")) +
tm_layout(legend.position = c("left", "bottom"))
...
leaflet(data = departements) %>% 
  addTiles() %>% 
  addPolygons(fillColor = ~colorQuantile("Blues", departements@data$Sarkozy2.exp, n = 6)(Sarkozy2.exp), color = "transparent", fillOpacity = 0.6, popup = ~htmlEscape(paste0("Sarkozy (2e tour) : ", round(Sarkozy2.exp, 1)))) 
```
